...
export const BookingModal = ({ doctor, onClose }) => {
  const { bookAppointment, setActiveView, appointments = [] } = useApp();
  
  // Date choices (Today + 3 Days)
  ...
  // Real-time slot counts map from backend database
  const [slotDataMap, setSlotDataMap] = useState({});
  const [liveQueueList, setLiveQueueList] = useState([]);
  const [imgError, setImgError] = useState(false);

  // Clean formatted dates array
  const dateOptions = [
    { label: 'Today', dateStr: todayStr, dayName: 'Aug 28' },
    { label: 'Tomorrow', dateStr: '2026-08-29', dayName: 'Aug 29' },
    { label: 'Sunday', dateStr: '2026-08-30', dayName: 'Aug 30' },
    { label: 'Monday', dateStr: '2026-08-31', dayName: 'Aug 31' },
  ];

  // 1. Fetch Real-Time Slot Counts & Queue from Backend Database
  const loadRealtimeCounts = async () => {
    try {
      ...
      if (data && data.success) {
        ...
        // Also combine with local appointments in state for immediate feedback
        const localDocAppts = (appointments || []).filter(
          a => (a.doctorId === doctor.id || a.doctorName === doctor.name) && 
               a.date === selectedDate && 
               ['upcoming', 'booked', 'waiting', 'in_consultation'].includes(a.status?.toLowerCase())
        );

        const mergedMap = {};

        (doctor.availableSlots || ['09:30 AM', '11:00 AM', '02:30 PM', '04:15 PM']).forEach(slot => {
          const dbCount = counts[slot]?.patientsBooked || 0;
          const localCount = localDocAppts.filter(a => a.timeSlot === slot).length;
          const totalBooked = Math.max(dbCount, localCount);

          mergedMap[slot] = {
            patientsBooked: totalBooked,
            expectedPosition: totalBooked + 1,
          };
        });

        setSlotDataMap(mergedMap);
      }

      // Fetch live session queue for queue visualization
      const queueRes = await fetchLiveQueueApi(doctor.id, doctor.name, selectedDate);
  ...
  // 2. Poll & Listen for WebSockets Events in Real Time
  useEffect(() => {
    loadRealtimeCounts();

    // Join WebSockets session room for live updates without page refresh
    ...
    const handleSocketUpdate = (data) => {
      console.log('⚡ [BookingModal] Real-time WebSockets event:', data);
      loadRealtimeCounts();
    };
    ...
    const timer = setInterval(loadRealtimeCounts, 2000);
  ...
  }, [doctor.id, doctor.name, selectedDate, selectedSlot, appointments]);

  const currentSlotInfo = slotDataMap[selectedSlot] || { patientsBooked: 0, expectedPosition: 1 };
      ...
      <div className="bg-white border border-slate-200/90 w-full max-w-2xl rounded-3xl p-6 sm:p-8 shadow-2xl relative max-h-[92vh] overflow-y-auto">
        ...
        {!isSuccess ? (
          <div className="space-y-6">
            ...
            {/* 1. DOCTOR HEADER CARD (Clean White Card) */}
            <div className="bg-slate-50/80 border border-slate-200/80 p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                {/* 64x64 Doctor Image with Fallback */}
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <p className="text-xs font-semibold text-slate-600 truncate">
                    {doctor.specialization} • {doctor.hospitalName || 'KMC Hospital'}
                  </p>
                  <div className="flex items-center gap-2 pt-0.5">
                    <span className="text-[11px] font-extrabold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Available Today
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-2xl text-center transition-all border ${
                        isSelected
                          ? 'bg-gradient-to-r from-brand-600 to-cyan-600 text-white border-transparent shadow-sm'
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {(doctor.availableSlots || ['09:30 AM', '11:00 AM', '02:30 PM', '04:15 PM']).map((slot) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition-all cursor-pointer relative ${
                        isSelected
                          ? 'border-brand-600 bg-brand-50/80 shadow-2xs ring-1 ring-brand-500'
                          : 'border-slate-200 bg-white hover:bg-slate-50'
                      }`}
                    >
                      <div className="flex items-center justify-between">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/90 space-y-2 text-xs">
              ...
              <div className="space-y-1 font-semibold text-slate-700">
                ...
                <p>🕘 Session Time: <strong className="text-slate-900">{selectedSlot}</strong></p>
                <p className="text-slate-600 pt-1">
                  💡 <strong>{patientsAlreadyBooked}</strong> {patientsAlreadyBooked === 1 ? 'patient has' : 'patients have'} already booked this session.
                </p>
              </div>

              {/* Anonymized Queue Visualization */}
            ...
            <div className="space-y-2 pt-2">
              <button
                ...
                className="w-full bg-gradient-to-r from-brand-600 to-cyan-600 hover:from-brand-700 hover:to-cyan-700 text-white font-extrabold text-sm py-3.5 rounded-2xl shadow-md shadow-brand-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                <span>{isSubmitting ? 'Reserving OPD Token...' : 'Confirm Appointment'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>

              <p className="text-[11px] text-center text-slate-500 font-semibold">
                ₹{doctor.consultationFee || 750} consultation fee • Secure booking • Live queue tracking
              </p>
            </div>
          ...
          <div className="text-center py-6 space-y-5 animate-in fade-in zoom-in duration-300">
            ...
            <button
              ...
              className="w-full max-w-md bg-brand-600 hover:bg-brand-700 text-white font-extrabold text-xs py-3.5 rounded-2xl shadow-sm transition-all"
            >
